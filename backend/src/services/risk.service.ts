...
export class RiskService {
   ...
   */
  async getRisks(filters: RiskFilters): Promise<{
    data: RiskData[];
    meta: {
      total: number;
      page: number;
   ...
   */
  async getTopRisks(tahun: number, n: number = 15): Promise<RiskData[]> {
    const query = `
      SELECT
        r.id,
        r.id_risiko,
   ...
   */
  async getRiskById(id: string): Promise<RiskData | null> {
    const query = `
      SELECT
        r.id,
        r.id_risiko,
   ...
   * Create new risk
   */
  async createRisk(data: Partial<RiskData>, userId: string): Promise<RiskData> {
    const {
      id_risiko,
      tahun,
      direktorat_id,
  ...
  async updateRisk(id: string, data: Partial<RiskData>): Promise<RiskData> {
    ...
    updateableFields.forEach((field) => {
      if (data[field as keyof RiskData] !== undefined) {
        fields.push(`${field} = $${paramIndex}`);
        values.push(data[field as keyof RiskData]);
        paramIndex++;
      }
    });
    ...
    if (!risk) throw new Error('Failed to update risk');
    return risk;
  }
  ...
  async deleteRisk(id: string): Promise<void> {
    ...
    const result = await this.pool.query(query, [id]);
    if (result.rowCount === 0) throw new Error('Risk not found or already deleted');
  }

  /**
   * Get risk level reference
   */
  async getRiskLevelRef(): Promise<RiskLevelRef[]> {
    const query = `
   ...
   */
  async getRiskStats(tahun: number): Promise<{
    total: number;
    byLevel: { [key: string]: number };
    topDirektorat: Array<{ direktorat: string; count: number }>;
  }> {
    const [totalResult, byLevelResult, topDirResult] = await Promise.all([
      this.pool.query(
        `SELECT COUNT(*) as total FROM pkpt.risk_data WHERE tahun = $1 AND deleted_at IS NULL`,
        [tahun]
      ),
      this.pool.query(
        `SELECT level_inherent, COUNT(*) as count FROM pkpt.risk_data
         WHERE tahun = $1 AND deleted_at IS NULL
         GROUP BY level_inherent ORDER BY level_inherent`,
        [tahun]
      ),
      this.pool.query(
        `SELECT r.direktorat_nama as direktorat, COUNT(*) as count FROM pkpt.risk_data r
         WHERE r.tahun = $1 AND r.deleted_at IS NULL
         GROUP BY r.direktorat_nama ORDER BY count DESC LIMIT 5`,
        [tahun]
      ),
    ]);
   ...
   */
  private _enrichRiskData(row: any): RiskData {
    return {
      ...row,
