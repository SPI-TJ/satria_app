...
export async function getActivityLog(req: Request, res: Response) {
  try {
    ...
    const modulRes = await query<{ modul: string }>(
      `SELECT DISTINCT modul FROM auth.activity_log ORDER BY modul`,
    );
    const actionRes = await query<{ action: string }>(
...
export async function getActivityLogSummary(_req: Request, res: Response) {
  try {
    const [byModul, byAction, recent] = await Promise.all([
      query<{ modul: string; count: string }>(
      ...
      query<{ count: string }>(
        `SELECT COUNT(*) AS count FROM auth.activity_log WHERE created_at >= NOW() - INTERVAL '24 hours'`,
      ),
    ]);
