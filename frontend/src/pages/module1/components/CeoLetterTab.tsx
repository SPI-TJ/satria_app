/**
 * Tab CEO Letter — Surat Arahan Direksi (versi tab di PKPTPage)
 *
 * Sama dengan CeoLetterPage tapi:
 *   - Menerima `tahun` sebagai prop (mengikuti year selector PKPTPage)
 *   - Tanpa top panel (breadcrumb + year selector internal)
 */
import { useEffect, useMemo, useState } from 'react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import {
  FileText, Upload, Trash2, Plus, Save,
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6 space-y-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h2 className="text-base font-bold text-slate-800">CEO Letter — Surat Arahan Direksi {tahun}</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder={`Contoh: Arahan Pengawasan Direksi Tahun ${tahun}`}
            />
          </div>
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="input w-full"
                placeholder={`contoh: 001/DIR/IV/${tahun}`}
              />
            </div>
          </div>
          <div>
